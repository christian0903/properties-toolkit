...
import { PropertyOccurrence } from '../types/mgr-types';

/** Map from file path to the chosen value */
export type MultiValueChoices = Map<string, any>;
...
export class MultiValueChoiceModal extends Modal {
  ...
  onOpen(): void {
    ...
    contentEl.createEl('p', {
      text: `${this.multiValueFiles.length} fichier(s) ont plusieurs valeurs. Sélectionnez la valeur à garder pour chaque fichier.`,
    });

    const list = contentEl.createEl('div', { cls: 'pt-preview-list' });

    for (const occ of this.multiValueFiles) {
      const values: any[] = Array.isArray(occ.currentValue) ? occ.currentValue : [occ.currentValue];
      ...
      for (let i = 0; i < values.length; i++) {
        ...
        const radio = label.createEl('input') as HTMLInputElement;
        radio.type = 'radio';
